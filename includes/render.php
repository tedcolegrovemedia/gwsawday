...
// escaped with htmlspecialchars() and image URLs are validated.
...
function render_phone_preview(array $build): string {
    ...
    $primary_key = (string)($build['colors']['primary'] ?? 'navy');
    $accent_key  = (string)($build['colors']['accent']  ?? 'orange');
    if (!is_valid_color_key($primary_key)) $primary_key = 'navy';
    if (!is_valid_color_key($accent_key))  $accent_key  = 'orange';
    $primary_hex = color_hex($primary_key);
    $accent_hex  = color_hex($accent_key);
    ...
    $style_vars = '--app-primary:' . $primary_hex . ';--app-accent:' . $accent_hex . ';';

    ob_start(); ?>
    <div class="phone">
      <div class="phone-frame">
        <div class="phone-notch"></div>
        <div class="phone-screen" data-font="<?= $h($font_key) ?>" style="<?= $h($style_vars) ?>">
          ...
          <div class="app-content">
            <div class="app-cards">
              <?php foreach ($cards as $c):
                $c_img  = (string)($c['image_url'] ?? '');
                if (strpos($c_img, 'https://') !== 0) $c_img = '';
              ?>
                <div class="app-card<?= $c_img ? '' : ' empty' ?>" style="<?= $c_img ? 'background-image:url(\'' . $h($c_img) . '\')' : '' ?>">
          ...
          <div class="app-footer">
            ...
            <?php if ($credit): ?>
              <div class="app-credit">
                Header: <?php if ($creditU): ?><a href="<?= $h($creditU) ?>" target="_blank" rel="noopener"><?= $h($credit) ?></a><?php else: ?><?= $h($credit) ?><?php endif; ?>
